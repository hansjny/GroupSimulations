...
class Group:
    ...
    def removeExcessMembers(self, maxSize):
        ...
        while (len(self.members) > maxSize):
                #$find all 1 connected, 2 connected, 3 connected, operate with treshold
            nodes = self.findNConnected(n)
            disturber = None
            ...
            self.members.remove(disturber)
            print("deletera!")

            groupCollection.newGroup(disturber)
    ...
    def findNConnected(self, n):
        leastConnected = []
        for node in self.members:
            neighbours = node.getNeighbourCount()
            if (neighbours == n): 
                leastConnected.append(node)
